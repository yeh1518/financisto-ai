...
public class TemplateGenerator {
    ...
    private static final String RESPONSE_SCHEMA =
            ...
            +   "\"required\":[\"template\",\"account_id\",\"is_income\",\"sample_amount\",\"confidence\"],"
            +   "\"properties\":{"
    ...
    private static final String SYSTEM_PROMPT =
            ...
            + "- **樣板要短，只留兩種文字**，其餘一律用 {{*}} 略過：\n"
            + "  (1) 辨識錨點——足以認出「這是哪家的哪種通知」的最短片語（例：甲銀行、乙卡）；\n"
            + "  (2) 貼身定界字——緊貼佔位符前後、用來框出欄位邊界的那幾個字（例：末四碼、元、商店名稱:）。\n"
            + "  銀行隨時會改的長尾文字**一律不要照抄**：免責聲明、客服電話、分期利率、「實際請款金額\n"
            + "  以帳單所列為準」「謹慎理財信用至上」這類句子，用 {{*}} 蓋掉或讓樣板在最後一個欄位就結束。\n"
            + "  理由：app 是用「找得到子字串」比對，不必涵蓋整則通知；抄得越多，銀行改一個字就整條失效。\n"
            + "- 中段的變動敘述要**大方用 {{*}}**：兩個欄位之間的文字（「感謝您使用…刷卡」「於08/22 13:18刷」）\n"
            + "  只要不是定界字就用一個 {{*}} 帶過，不要逐字抄。同一家銀行的刷卡通知常有好幾種寫法\n"
            + "  （「您好，您末四碼X於MM/DD HH:MM刷台幣N元」與「信用卡末四碼X刷卡通知…金額台幣N元」），\n"
            + "  只留錨點與定界字時一條樣板就能通吃。\n"
            ...
            // 刻意不要 category_id：一條樣板通吃一張卡的所有消費通知，而同一張卡會刷保費、
            // 電影、加油……綁死任何一個分類都會把其餘全部記錯，而且是靜默的（樣板綁的分類會
            // 無條件蓋過 payee 記憶，見 SmsTransactionProcessor 那段）。模型只看得到一則樣本，
            // 沒有辦法從中判斷「這類通知的用途單不單一」。分類交給 payee.lastCategoryId 學；
            // 真的用途單一（停車費、電信費）由使用者自己去樣板編輯器指定。
    ...
    public GeneratedTemplate generate(String title, String body) throws GenerateException {
        ...
        for (int attempt = 1; attempt <= 2; attempt++) {
            ...
            String repaired = t.template;
            // 先試裁掉尾巴的短版：過得了同一關驗證就用短的（銀行改長尾文案時才不會整條失效）
            String trimmed = trimTail(repaired);
            if (!trimmed.equals(repaired)) {
                t.template = trimmed;
                if (validate(t, body) != null) t.template = repaired;
            }
            String problem = validate(t, body);
            StringBuilder logged = new StringBuilder(t.template);
            if (!repaired.equals(raw)) {
                logged.append("\n（空白已依原文修正，模型原本產出：").append(raw).append("）");
            }
            if (!t.template.equals(repaired)) {
                logged.append("\n（尾巴長尾文字已裁掉，裁切前：").append(repaired).append("）");
            }
            log(body, attempt, logged.toString(), describe(t), problem);
    ...
    /** 尾端裁切時用來當結束標記的界字：碰到其中一個就可以收手。 */
    private static final String TAIL_DELIMS = "，,。;；:：、（）()「」【】　 ";
    /** 捕捉型佔位符要留長一點的結束標記；數值型佔位符本身的 regex 已有邊界，留短的就夠。 */
    private static final int TAIL_CAP_CAPTURE = 8;
    private static final int TAIL_CAP_PLAIN = 4;

    /**
     * 裁掉樣板尾巴的長尾固定文字。
     *
     * 引擎用 find() 找子字串、不要求涵蓋整則通知，所以「最後一個欄位」之後的文字對比對毫無
     * 貢獻——卻正好是銀行最常改的地方（免責聲明、客服電話、分期利率、謹慎理財標語）。抄了
     * 就等於把「這家銀行今天的公關文案」寫進比對條件，改一個字整條樣板失效。實地案例：某家銀行
     * 的刷卡通知有三種寫法，兩條長樣板各只吃一種，第三種掉了。
     *
     * 留下貼身的結束標記（{{e}} 這類非貪婪捕捉沒有標記會退化成只抓一個字元，見
     * {@link #findDegenerateCapture}），其餘丟掉。尾端就是 {{*}} 的話連它一起丟——它蓋的
     * 正是不需要比對的那段。
     *
     * 只做確定性裁切，裁完一定要再過 {@link #validate} 回測；沒過就用未裁切的版本。
     */
    static String trimTail(String template) {
        if (template == null) return null;
        java.util.regex.Matcher m = PLACEHOLDER.matcher(template);
        int lastEnd = -1, lastStart = -1;
        String lastPh = null;
        while (m.find()) { lastStart = m.start(); lastEnd = m.end(); lastPh = m.group(); }
        if (lastPh == null) return template;              // 沒有佔位符：不動，交給 validate 擋
        if ("{{*}}".equals(lastPh)) return template.substring(0, lastStart);

        boolean capture = "{{u}}".equals(lastPh);
        for (String ph : NEEDS_ANCHOR) if (ph.equals(lastPh)) capture = true;
        int cap = capture ? TAIL_CAP_CAPTURE : TAIL_CAP_PLAIN;

        String tail = template.substring(lastEnd);
        if (tail.length() <= 1) return template;          // 本來就只有一個字，沒什麼可裁
        int keep = Math.min(cap, tail.length());
        // 以「第一個界字」為界：非貪婪捕捉碰到一個逗號就停得住，留越短越吃得下同家銀行的其他寫法
        for (int i = 0; i < keep; i++) {
            if (TAIL_DELIMS.indexOf(tail.charAt(i)) >= 0) { keep = i + 1; break; }
        }
        if (keep >= tail.length()) return template;
        return template.substring(0, lastEnd + keep);
    }
    ...
    private GeneratedTemplate callModel(String title, String body, String retryHint)
        ...
        try {
            ...
            // 查找鍵一律用通知的真實標題，不問模型：執行期就是拿這個值去 `? LIKE title` 查樣板，
            // 而模型看不到「標題其實是空的」這件事，會從內文挖一段當標題（實際踩過：某張卡的
            // 通知標題為空、內文開頭是「【信用卡消費通知】」，模型拿它當 title_key，存進去之後
            // 執行期拿空標題去查永遠對不上，那條樣板一次都沒生效過）。字串比對的事歸程式。
            t.titleKey = title == null ? "" : title;
